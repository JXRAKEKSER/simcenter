    ...
    if form.validate_on_submit():
        ...
        with open(csv_path, 'w+') as csv_file:
            ...
            rows = [ {fields[i]: value for i, value in enumerate(values)}
                    for values in get_module_values(module) if values[7] == form.date.data]
...
def operators():
    ...
    page = request.args.get('page', 1, type=int)
    pagination = data.Operators.query.filter_by()\
                                     .paginate(page, per_page=10, error_out=False)

    return render_template('operators.html',
                           page_title=str() + ' operators',
                           len=len,
                  #         offices=data.Office.query,
                           pagination=pagination,
                           usersp=pagination.items,
