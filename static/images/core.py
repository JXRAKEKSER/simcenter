...
@core.route('/monitor/<string:room>')
def monitor(room):
    a = datetime.datetime.now()
    b= timedelta(hours=2,minutes=55)
    c = a+b
    ...
    test4 = data.Stud_access.query.filter(data.Stud_access.time_begin > timez).filter(data.Stud_access.room == room).order_by(data.Stud_access.time_begin).all()
    return render_template('tablo2.html', test4=test4, rr=rr)
...
def pult():
    ...
    while i<=max:
        ...
        if i % (len(room_1)+len(room_2)+len(room_3)) == 0:
            cx = rx
            mx = fx
...
def regist():
    return render_teplate('regist.html')
