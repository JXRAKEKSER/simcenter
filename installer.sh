...
then
  ...
  echo "##### Running AKT $VERSION #####"
  if [ -f run.py ]
  then
    python run.py "${@:2}"
  else
    echo "Error: can not find AKT run.py"
  fi
elif [ "$1" == "--test" ]
then
  if [ -d installiation/ ]
...
then
  ...
  echo -e "\t $0 --run \t\t to run AKT"
  echo -e "\t $0 --test \t\t to run AKT tests"
  echo -e "\t $0 --migration \t to run AKT migration"
